...
async function makeTransparent(input, output) {
  try {
    ...
    function tryEnqueue(x, y) {
      ...
      const isWhite = rgba.r > 200 && rgba.g > 200 && rgba.b > 200;
      const isBg = (rgba.r > 120 && rgba.g > 120 && rgba.b > 120 && sat < 50); // Very aggressive catch for light grays
      
      if (colorDistance(c, bgColor) < 90 || isBg || isWhite) {
        visited[idx] = 1;
        queue.push({ x, y });
      }
    }
    ...
    // Multiple passes of fringe removal (increased to 8 passes to deeply erode white halos)
      ...
      for (let y = 1; y < h - 1; y++) {
        for (let x = 1; x < w - 1; x++) {
          ...
          if (img.bitmap.data[pIdx + 3] > 0) {
            ...
            const isWhiteFringe = (r > 110 && g > 110 && b > 110) && (saturation < 60);
            const isVeryWhite = r > 180 && g > 180 && b > 180;
            ...
            if (hasTransparentNeighbor && (isWhiteFringe || isVeryWhite)) {
               toRemove.push(pIdx);
            }
          }
        }
      }
      for (const pIdx of toRemove) {
...
async function main() {
  ...
  // Re-process Knight, Qilin, and Wizard from src/imports as they weren't in the uploaded media list (they were static)
  ...
  for (const file of files) {
    if (fs.existsSync(file.in)) {
      ...
      console.log(`Skipping missing input: ${file.in}`);
    }
  }
}
