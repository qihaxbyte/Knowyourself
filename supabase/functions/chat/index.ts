...
// ──────────────────────────────────────────────
function buildSystemPrompt(guideId: string, results: string): string {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { guideId, result, history } = await req.json();

    if (!guideId || !history || !Array.isArray(history)) {
      return new Response(
        JSON.stringify({ error: "Missing required fields: guideId, history" }),
