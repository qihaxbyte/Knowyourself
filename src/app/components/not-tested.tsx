...
export default function PreTestPrompt({ onStart }: { onStart: () => void }) {
  return (
    <div className="flex min-h-screen w-full flex-col items-center justify-center p-6 text-center" style={{ background: "#FAF7F0", fontFamily: "Inter, sans-serif" }}>
      ...
      <button onClick={onStart} className="mt-8 inline-flex items-center gap-2 rounded-xl px-8 py-4 text-sm font-bold text-white shadow-lg transition hover:scale-105 hover:brightness-110" style={{ background: "linear-gradient(135deg, #2D6A4F, #C9A84C)" }}>
